...
public partial class Home : FluxorComponent
{
  ...
  private bool _usa_domini_specifici;
  ...
  private async Task SendRequest()
  {
    ...
    if (_usa_domini_specifici && !_domains.Any())
    {
      await DialogService.ShowMessageBox("Domini specifici non specificati", 
        "Quando attivato l'uso di domini soecifici è necessario definire almeno un dominio.");
      return;
    }

    _config.sites_search_restriction = _usa_domini_specifici ? _domains : [];
