...
class SourcesFormatter():
    ...
    def format_riepilogo(sources: List[SearchEngResult], include_snippet: bool = False):
        ...
        for i, source in enumerate(sources, 1):
            if include_snippet:
                formatted_text += f"<a href='{source['url']}' target='_blank'>[{source['num_source']}]</a> {source['title']} - {source['snippet']}\n\n"
            else:
                formatted_text += f"<a href='{source['url']}' target='_blank'>[{source['num_source']}]</a> {source['title']}\n\n"
